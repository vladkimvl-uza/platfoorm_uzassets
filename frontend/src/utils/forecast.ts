...
export const FORECAST_MODELS: ForecastModelMeta[] = [
  {
    ...
    desc: i18nKey("Проекция по последнему годовому темпу роста (YoY последнего факта). Доп. данные не нужны."),
    inputs: [],
  },
  {
    id: "cagr", label: "CAGR",
    desc: i18nKey("Среднегодовой темп роста за выбранный базовый период. Укажите базовые годы."),
    inputs: ["cagrRange"],
  },
  {
    id: "linear", label: i18nKey("Линейный тренд"),
    desc: i18nKey("Экстраполяция по методу наименьших квадратов. Укажите глубину истории."),
    inputs: ["linearWindow"],
  },
  {
    id: "manual", label: i18nKey("Ручные допущения"),
    desc: i18nKey("Вы задаёте процент роста выручки на каждый прогнозный год."),
    inputs: ["growthPct"],
  },
  {
    id: "flat", label: i18nKey("Без изменений"),
    desc: i18nKey("Удержание последнего факта без роста. Доп. данные не нужны."),
    inputs: [],
  },
];
