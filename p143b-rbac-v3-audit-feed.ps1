# =====================================================================
# p143b-rbac-v3-audit-feed.ps1   (RBAC v3 session 4 - part 2)
# =====================================================================
# Implements AuditFeedPage as GitHub-style chronological feed.
# Uses existing endpoints (prefix /admin/audit):
#   GET /admin/audit/events       -> paginated list with filters
#   GET /admin/audit/events/{id}  -> single event detail (diff + payload)
#   GET /admin/audit/export.csv   -> CSV export (requires audit.admin)
#
# UI features:
#   - sidebar filters: period (24h / 7d / 30d / all), module, action, critical
#   - search input
#   - feed: each event = card with actor-avatar + severity dot + human-readable text
#   - relative time + tooltip with exact timestamp
#   - <details> for diff/payload (lazy-loaded on expand)
#   - CSV export button
...
function Write-NewFile($path, $content, $label, [switch]$Overwrite) {
    ...
    if (Test-Path -LiteralPath $path) {
        ...
        Copy-Item -LiteralPath $path -Destination "$path.bakP143b.$stamp" -Force
    }
...
# ───────────────────────────────────────────────────────────────────────
# [1/2] api/rbacV3.ts — add auditApi
# ───────────────────────────────────────────────────────────────────────
$apiAddon = @'
...
if ($apiSrc.Contains('export const auditApi')) {
    Write-Host "[*] [1/2] api: auditApi already added" -ForegroundColor DarkGray
} else {
    Copy-Item -LiteralPath $apiPath -Destination "$apiPath.bakP143b.$stamp" -Force
    Write-Host "[*] [1/2] api/rbacV3.ts: adding auditApi" -ForegroundColor Yellow
    Write-File $apiPath ($apiSrc + $apiAddon)
    Write-Host "    OK" -ForegroundColor Green
}

# ───────────────────────────────────────────────────────────────────────
# [2/2] views/rbac-v3/AuditFeedPage.vue
# ───────────────────────────────────────────────────────────────────────
$page = @'
...
'@
Write-NewFile (Join-Path $root "$fe\views\rbac-v3\AuditFeedPage.vue") $page "[2/2] views/rbac-v3/AuditFeedPage.vue" -Overwrite
...
Write-Host " p143b COMPLETE - Audit feed live" -ForegroundColor Green
Write-Host "===================================================================" -ForegroundColor Green
Write-Host ""
Write-Host "Test (Ctrl+Shift+R):" -ForegroundColor Cyan
Write-Host "  1. /admin/rbac-v3 -> Аудит tab" -ForegroundColor White
Write-Host "  2. Left sidebar: period selector (Сутки / 7 дней / 30 дней / Всё)" -ForegroundColor White
Write-Host "  3. Module filter dropdown + 'Только critical' checkbox" -ForegroundColor White
Write-Host "  4. 'Экспорт в CSV' button at bottom of sidebar" -ForegroundColor White
Write-Host "  5. Right: chronological feed grouped by day (Сегодня / Вчера / dates)" -ForegroundColor White
Write-Host "  6. Each event: actor avatar + severity dot + human-readable text" -ForegroundColor White
Write-Host "  7. Click 'подробности' on events with diff/payload to expand" -ForegroundColor White
Write-Host "  8. Pagination at bottom if >50 events" -ForegroundColor White
