...
async function remove(u: ModeratorUser) {
  const ok = await confirmDialog({
    ...
    message: u.id === auth.user?.id
      ...
      : t("«{name}» перестанет получать заявки и не сможет их согласовывать — ни в платформе, ни из Telegram. Роли и остальные права не меняются, вернуть в модераторы можно в любой момент.", { name: nameOf(u) }),
