...
- Swagger UI: `https://<хост>/api/docs` — все эндпоинты, схемы, кнопка **Authorize** (вставить API-ключ или Bearer-токен) и «Try it out».
- ReDoc: `https://<хост>/api/redoc`
- OpenAPI-спека (для Postman/кодогенерации): `https://<хост>/api/openapi.json`
...
`{code}` — **код компании ИЛИ ИНН** (см. раздел 4). Т.е. можно тянуть прямо по
ИНН: `…/companies/200837914/indicators` ≡ `…/companies/ung/indicators`. Год — 4 цифры (`2024`).

> Список всех ИНН одним запросом: `GET /api/companies` → каждый элемент содержит `code`, `inn`, `name_ru`, `sector_*`.
> Типовой сценарий интеграции: `GET /api/companies` → собрать все `inn` → дальше дёргать `…/companies/{inn}/…`.
