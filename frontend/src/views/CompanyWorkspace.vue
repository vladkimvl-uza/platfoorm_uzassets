...
import ProjectsPrintReport from "@/components/ProjectsPrintReport.vue";
import { statusUpdatesApi, HEALTH_META, type StatusUpdate } from "@/api/statusUpdates";
import minfinLogoUrl from "@/assets/minfin-logo.png";
import uzassetsLogoWideUrl from "@/assets/uzassets-logo-wide.png";
...
// =====================================================================
// Печатный документ «сводный обзор» (проекты/задачи + ход проекта)
// =====================================================================

const printOpen = ref(false);
const printBusy = ref(false);
const printStatuses = ref<Record<string, StatusUpdate | null>>({});

// Счётчики берём из реально отрисованных групп, чтобы шапка не завышала число
// задач (одиночные задачи без проекта в структуру направление→проект не входят).
const printTotals = computed(() => ({
  projects: projItems.value.length,
  tasks: printGroups.value.reduce(
    (sum, g) => sum + g.projects.reduce((s, p) => s + p.tasks.length, 0),
    0,
  ),
}));

// Группы: направление → проекты → задачи (+ метки/цвета статусов, последний ход).
const printGroups = computed(() => {
  const tasksByProj = new Map<string, TaskBrief[]>();
  for (const t of taskItems.value) {
    const pid = (t as any).project_id;
    if (!pid) continue;
    (tasksByProj.get(pid) || tasksByProj.set(pid, []).get(pid)!).push(t);
  }
  const byDir = new Map<string, ProjectBrief[]>();
  for (const p of projItems.value) {
    const key = String(p.direction || "").toLowerCase() || "__none__";
    (byDir.get(key) || byDir.set(key, []).get(key)!).push(p);
  }
  const groups = Array.from(byDir.entries()).map(([key, ps]) => {
    const meta = key === "__none__" ? null : (_DIRS_META[key] || null);
    return {
      key,
      label: meta?.label || "Без направления",
      ...
      projects: ps
        ...
        .map(p => ({
          ...
          status: printStatuses.value[p.id] || null,
          tasks: (tasksByProj.get(p.id) || [])
            .slice()
            .sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0))
            .map(t => ({
              task: t,
...
async function openPrintDoc() {
  if (printBusy.value) return;
  printBusy.value = true;
  try {
    const projs = projItems.value;
    const results = await Promise.allSettled(
      projs.map(p => statusUpdatesApi.list("project", p.id)),
    );
    const map: Record<string, StatusUpdate | null> = {};
    results.forEach((r, i) => {
      if (r.status === "fulfilled" && r.value.length) {
        const latest = r.value
          .slice()
          .sort((a, b) => String(b.created_at || "").localeCompare(String(a.created_at || "")))[0];
        map[projs[i].id] = latest;
      } else {
        map[projs[i].id] = null;
      }
    });
    printStatuses.value = map;
...
function closePrintDoc() {
  ...
  document.body.classList.remove("pdoc-open");
}
...
// ── Экспорт в Word (.doc) — формируем Word-совместимый HTML с вшитыми base64
// логотипами; шапка и блоки проектов — таблицами (надёжно открывается в MS Word). ──
const printExporting = ref(false);

function _esc(s: any): string {
  return String(s ?? "")
    .replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
}
function _wDate(s: string | null | undefined): string {
  if (!s) return "—";
  const d = new Date(s);
  if (isNaN(d.getTime())) return "—";
  return d.toLocaleDateString("ru-RU", { day: "2-digit", month: "2-digit", year: "numeric" });
}
function _wPct(v: any): number {
  const n = Number(v ?? 0);
  return isNaN(n) ? 0 : Math.max(0, Math.min(100, Math.round(n)));
}
function _wPlural(n: number, a: string, b: string, c: string): string {
  const m10 = n % 10, m100 = n % 100;
  if (m10 === 1 && m100 !== 11) return a;
  if (m10 >= 2 && m10 <= 4 && (m100 < 10 || m100 >= 20)) return b;
  return c;
}
function _wHealth(h: StatusUpdate["health"]): { label: string; color: string } {
  return (h && HEALTH_META[h]) ? HEALTH_META[h] : { label: "Без оценки", color: "#8A8A8A" };
}
async function _assetDataUrl(url: string): Promise<string> {
  try {
...
function _buildWordHtml(minfin: string, uza: string): string {
  const cName = _esc(company.value?.name_ru || company.value?.name || "Компания");
  const sName = _esc((sector.value as any)?.name_ru || "—");
  const t = printTotals.value;
  const stamp = new Date().toLocaleDateString("ru-RU", { day: "2-digit", month: "long", year: "numeric" });

  const leftLogo = minfin
    ? `<img src="${minfin}" style="height:48px"/>`
    : `<div class="mt1">O'ZBEKISTON RESPUBLIKASI</div><div class="mt2">IQTISODIYOT VA MOLIYA</div><div class="mt2">VAZIRLIGI</div>`;
  const rightLogo = uza ? `<img src="${uza}" style="height:30px"/>` : `<b>UzAssets</b>`;

  let body = "";
  for (const g of printGroups.value) {
    body += `<p class="dir" style="border-left:4px solid ${g.color}"><span style="color:${g.color}">${_esc(g.label)}</span> <span class="dcount">&nbsp;${g.projects.length} ${_wPlural(g.projects.length, "проект", "проекта", "проектов")}</span></p>`;
    g.projects.forEach((row, i) => {
      const p: any = row.project;
      const tasksRows = row.tasks.length
        ? row.tasks.map(rt => `<tr><td class="tdot" style="color:${rt.statusColor}">●</td><td>${_esc(rt.task.title)}</td><td style="color:${rt.statusColor}">${_esc(rt.statusLabel)}</td><td class="tdue">${rt.task.due_date ? _wDate(rt.task.due_date) : "—"}</td></tr>`).join("")
        : `<tr><td></td><td colspan="3" class="muted">Задач нет</td></tr>`;
      const flow = row.status
        ? `<div class="fh" style="color:${_wHealth(row.status.health).color}">● ${_esc(_wHealth(row.status.health).label)}</div><div class="fb">${_esc(row.status.body)}</div><div class="fm">${_wDate(row.status.created_at)}${row.status.author_name ? " · " + _esc(row.status.author_name) : ""}</div>`
        : `<div class="muted">нет записей</div>`;
      body += `<table class="proj"><tr>`
        + `<td class="pmain"><p class="ptitle"><b>${i + 1}. ${_esc(p.title)}</b> &nbsp;<span style="color:${row.statusColor}">[${_esc(row.statusLabel)}]</span></p>`
        + `<p class="pmeta">${_wPct(p.progress_percent)}% · задач ${p.tasks_done ?? 0}/${p.tasks_total ?? row.tasks.length}${p.due_date ? " · срок " + _wDate(p.due_date) : ""}${p.assignee_name ? " · " + _esc(p.assignee_name) : ""}</p>`
        + `<table class="tasks">${tasksRows}</table></td>`
        + `<td class="pflow"><p class="ftitle">Ход проекта</p>${flow}</td>`
        + `</tr></table>`;
    });
  }
  ...
  return `<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40"><head><meta charset="utf-8">`
    ...
    + `body{font-family:'Segoe UI',Arial,sans-serif;font-size:10.5pt;color:#14171F}`
    + `table{border-collapse:collapse}`
    + `.head{width:100%;border-bottom:2px solid #4B4A9A}.head td{vertical-align:middle}`
    + `.ept{font-size:11pt;font-weight:bold;color:#4B4A9A;letter-spacing:1px;text-align:center}`
    + `.mt1{font-size:6pt;color:#666;letter-spacing:1px}.mt2{font-size:10pt;font-weight:bold}`
    + `.cname{font-size:17pt;font-weight:bold;padding-top:8px}`
    + `.sector{text-align:right;color:#8A8C99;font-size:9pt;text-transform:uppercase}`
    + `.summary{margin:8px 0 14px;color:#4A4D5C;font-size:10pt}`
    + `.dir{margin:12px 0 4px;padding-left:8px;font-size:12pt;font-weight:bold;text-transform:uppercase}`
    + `.dcount{color:#9698A4;font-size:9pt;font-weight:normal}`
    + `.proj{width:100%;margin-bottom:6px}.pmain{vertical-align:top;width:73%;padding-right:10px}`
    + `.pflow{vertical-align:top;width:27%;border-left:1px solid #E6E7EE;padding-left:10px}`
    + `.ptitle{margin:0 0 2px;font-size:11pt}.pmeta{margin:0 0 4px;color:#7C7F8C;font-size:9pt}`
    + `.tasks{width:100%}.tasks td{font-size:9.5pt;padding:1px 6px 1px 0;vertical-align:top}`
    + `.tdot{width:10px}.tdue{color:#9698A4;text-align:right;width:70px}`
    + `.muted{color:#A9ABB5;font-style:italic;font-size:9pt}`
    + `.ftitle{margin:0 0 3px;font-size:8pt;font-weight:bold;color:#A1A3AE;text-transform:uppercase}`
    + `.fh{font-size:9.5pt;font-weight:bold;margin-bottom:2px}.fb{font-size:9pt;color:#3A3D48}.fm{font-size:8pt;color:#A1A3AE;margin-top:2px}`
    + `.foot{margin-top:16px;border-top:1px solid #E6E7EE;padding-top:6px;text-align:center;color:#A1A3AE;font-size:8pt}`
    + `</style></head><body>`
    + `<table class="head"><tr>`
    + `<td style="width:34%;text-align:left">${leftLogo}</td>`
    + `<td style="width:33%" class="ept">ЕДИНАЯ ПЛАТФОРМА<br/>ТРАНСФОРМАЦИИ</td>`
    + `<td style="width:33%;text-align:right">${rightLogo}</td></tr>`
    + `<tr><td colspan="2" class="cname">${cName}</td><td class="sector">${sName} · сводный обзор</td></tr></table>`
    + `<p class="summary"><b>${t.projects} ${_wPlural(t.projects, "проект", "проекта", "проектов")}</b> · ${t.tasks} ${_wPlural(t.tasks, "задача", "задачи", "задач")} · FY ${year.value} · по состоянию на ${stamp}</p>`
    + (body || `<p class="muted">Нет проектов за выбранный год.</p>`)
    + `<p class="foot">Единая платформа трансформации · UzAssets — сформировано ${stamp}</p>`
    + `</body></html>`;
}

async function exportWord() {
  if (printExporting.value) return;
  printExporting.value = true;
  try {
    if (!printOpen.value) await openPrintDoc();
    const [minfin, uza] = await Promise.all([
      _assetDataUrl(minfinLogoUrl),
      _assetDataUrl(uzassetsLogoWideUrl),
    ]);
    const html = _buildWordHtml(minfin, uza);
    const blob = new Blob(["﻿", html], { type: "application/msword" });
    const url = URL.createObjectURL(blob);
    const cname = String(company.value?.name_short || company.value?.name_ru || "компания")
      .replace(/[\\/:*?"<>|]+/g, "").trim();
    const a = document.createElement("a");
    a.href = url;
    a.download = `Сводный обзор — ${cname} — FY${year.value}.doc`;
    document.body.appendChild(a);
    a.click();
    a.remove();
    setTimeout(() => URL.revokeObjectURL(url), 4000);
  } finally {
    printExporting.value = false;
  }
}

onUnmounted(() => document.body.classList.remove("pdoc-open"));
...
<template>
  <div class="cw-page cw-shell">
    ...
    <template v-else-if="company">
      ...
      <!-- Печатный документ «сводный обзор» — оверлей поверх всего (teleport
           в body, чтобы при печати скрыть весь .uza-shell, см. print.css). -->
      <Teleport to="body">
        <div v-if="printOpen" class="pdoc-overlay">
          <div class="pdoc-toolbar">
            <span class="pdoc-tb-title">Документ · FY {{ year }}</span>
            <span class="pdoc-tb-spacer" />
            <button class="pdoc-tb-btn pdoc-tb-word" :disabled="printExporting" @click="exportWord">{{ printExporting ? "Готовлю…" : "Скачать Word" }}</button>
            <button class="pdoc-tb-btn pdoc-tb-print" @click="runPrint">Печать</button>
            <button class="pdoc-tb-btn" @click="closePrintDoc">Закрыть</button>
          </div>
          <div class="pdoc-scroll">
            <ProjectsPrintReport
              :company="company"
              :sector-name="(sector as any)?.name_ru || ''"
              :year="year"
              :groups="printGroups"
              :totals="printTotals"
            />
...
/* ── Печатный документ: оверлей предпросмотра (экран) ── */
.pdoc-overlay {
  position: fixed;
  inset: 0;
  z-index: 4000;
...
.pdoc-tb-title { font-size: 13px; font-weight: 600; letter-spacing: 0.01em; }
.pdoc-tb-spacer { flex: 1; }
.pdoc-tb-btn {
  font-size: 13px;
  font-weight: 600;
  color: #fff;
...
.pdoc-tb-btn:hover { background: rgba(255, 255, 255, 0.2); }
.pdoc-tb-print {
  background: #7F77DD;
  border-color: #7F77DD;
}
.pdoc-tb-print:hover { background: #6F66D2; }
.pdoc-tb-word {
  background: #1D9E75;
  border-color: #1D9E75;
}
.pdoc-tb-word:hover { background: #178A66; }
.pdoc-tb-word:disabled { opacity: 0.6; cursor: default; }
.pdoc-scroll {
  flex: 1;
  overflow: auto;
