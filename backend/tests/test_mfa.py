...
def test_generate_recovery_codes_count_and_uniqueness():
    from app.services.mfa_service import RECOVERY_CODES_COUNT, generate_recovery_codes
...
async def test_verify_login_challenge_happy_path(db, make_user):
    ...
    from app.services.mfa_service import _gen_login_code, _hash_bcrypt, verify_login_challenge
...
async def test_confirm_link_with_wrong_token_returns_none(db, make_user):
    ...
    await make_user(email="bad-token@example.com")
