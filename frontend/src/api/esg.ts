...
export interface ESGKpiManagerBrief {
  id: string;
  title: string;
...
export const esgApi = {
  ...
  async getEsgKpiManagers(companyId: string, year?: number): Promise<ESGKpiManagerBrief[]> {
    const r = await api.get<ESGKpiManagerBrief[]>("/esg/kpi-managers", {
