...
const directionsCovered = computed<{ id: string | null; label: string; count: number }[]>(() => {
  ...
  for (const t of filteredTasks.value) {
    const key = t.direction_label || "Без направления";
...
const STATUS_LABEL: Record<string, string> = {
  done: "Завершено", active: "В процессе", init: "Инициирование",
  new: "Не начато", review: "На согласовании",
...
const headerTitle = computed(() => {
  if (props.kind === "consultant") return props.consultant?.name || "Консультант";
  if (props.kind === "cell") return `${props.cellBoard?.name || "—"} × ${props.cellConsultant?.name || "—"}`;
  if (props.kind === "direction") return props.direction?.label || "Направление";
  return "Детали";
});
...
const headerKind = computed(() => {
  if (props.kind === "consultant") return "Консультант";
  if (props.kind === "cell") return "Связка компания × консультант";
  return "Направление";
});
  ...
  <div class="cdm-backdrop" @click.self="emit('close')">
    <div class="cdm-shell">
      ...
      <header class="cdm-header" :style="{ '--accent': headerAccentColor }">
        ...
        <button class="cdm-close" @click="emit('close')" aria-label="Закрыть">
      ...
      <div class="cdm-body">
        ...
        <aside v-if="kind !== 'cell'" class="cdm-aside">
          ...
          <template v-if="kind === 'consultant'">
            <section class="cdm-sec">
              <div class="cdm-sec-lbl">Компании ({{ companiesCovered.length }})</div>
            ...
            <section class="cdm-sec">
              <div class="cdm-sec-lbl">Направления ({{ directionsCovered.length }})</div>
              <div v-for="(d, i) in directionsCovered" :key="`dir-${i}`" class="cdm-mini-row">
                <span class="cdm-mini-name">{{ d.label }}</span>
          ...
          <template v-else-if="kind === 'direction'">
            <section class="cdm-sec">
              <div class="cdm-sec-lbl">Консультанты ({{ consultantsForDirection.length }})</div>
        ...
        <main class="cdm-main">
          <div class="cdm-main-h">
            <span class="cdm-main-h-t">Задачи · {{ filteredTasks.length }}</span>
          </div>
          <div class="cdm-task-list">
            ...
            >
              ...
              <div class="cdm-task-main">
                ...
                <div class="cdm-task-meta">
                  ...
                  <span v-if="t.direction_label"> · {{ t.direction_label }}</span>
            ...
            <div v-if="!filteredTasks.length" class="cdm-empty">Нет задач</div>
          </div>
        </main>
      </div>
    </div>
