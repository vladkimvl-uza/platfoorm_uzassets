...
# Прогноз требует больше поисков (цены на сырьё, курс, санкции, макро — по
# нескольким факторам сразу), поэтому отдельный инструмент с бОльшим лимитом.
WEB_SEARCH_TOOL_DEEP = {
    "type": "web_search_20250305",
    "name": "web_search",
    "max_uses": 10,
}
...
async def ai_kpi_analysis(
    ...
    _persona = {
        ...
        "forecast": (
            ...
            "интерпретировать, при необходимости КОРРЕКТИРОВАТЬ с учётом внешних и внутренних "
            "экономических факторов (web) и ЯВНО объяснять расхождения с моделью. Где method=none "
            "(нет истории) — обоснуй прогноз качественно и пометь низкую надёжность.\n"
            "0) ВНЕШНИЕ И ВНУТРЕННИЕ ФАКТОРЫ (ОБЯЗАТЕЛЬНЫЙ раздел ПЕРЕД таблицами, используй "
            "web_search для актуальных данных) — ТАБЛИЦЕЙ: Фактор | Текущее состояние/прогноз | "
            "Влияние на KPI (+/−) | Учтён в корректировке. Разбери минимум:\n"
            "  ВНЕШНИЕ: мировые цены на профильное сырьё/продукцию компании (для НГМК/АГМК — "
            "золото, серебро, медь, уран; нефть Brent/Urals, природный газ, металлы, хлопок, "
            "удобрения, урожай — по сектору); курс UZS/USD и девальвация; санкции и геополитика "
            "(экспортные рынки, логистика, доступ к технологиям/финансированию, вторичные санкции); "
            "мировой спрос и цены конкурентов; ставки ФРС/мировые ставки.\n"
            "  ВНУТРЕННИЕ: макро Узбекистана (рост ВВП, инфляция, ставка ЦБ РУз), тарифы на "
            "энергию/газ/воду, налоговая и таможенная политика, субсидии и госзаказ, курсовая "
            "политика, инвестпрограмма и локализация, доступность рабочей силы.\n"
            "Каждый значимый фактор ДОЛЖЕН отразиться в корректировке прогноза (или явно "
            "помечен как несущественный). Числа модели — базовый сценарий БЕЗ этих факторов; "
            "твоя добавленная ценность — наложить их и обосновать сдвиг.\n"
    ...
    # Прогноз анализирует внешние/внутренние факторы (сырьё, курс, санкции, макро) —
    # даём больше web-поисков; остальным режимам достаточно базового лимита.
    _web_tool = WEB_SEARCH_TOOL_DEEP if mode == "forecast" else WEB_SEARCH_TOOL
    _timeout = 225.0 if mode == "forecast" else 200.0
    try:
        text = await complete_once(
            system=system, prompt=prompt, model="ai-deep", max_tokens=8000,
            temperature=None, tools=[_web_tool], timeout=_timeout,
        )
    except Exception as e_web:  # noqa: BLE001
...
async def ai_bp_analysis(
    ...
    # 'ytd' = годовой факт из значения Q4 (кварталы БП хранятся нарастающим итогом).
    _SRC = {"nsbu": "НСБУ", "ytd": "нараст. итог (Q4)"}
    bp_blocks: list[str] = []
    for r in bp_rows:
        nm = r.get("name") or r.get("code") or "—"
        block = [f"## {nm}"]
    ...
    _persona = {
        "performance": (
            ...
            "- качество факта: помечай источник [НСБУ] (закрытый год) vs [нараст. итог (Q4)] vs ручной;\n"
