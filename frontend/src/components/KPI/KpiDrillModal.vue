...
function srcShort(s?: string | null): string {
  return ({ ytd: "кв.", nsbu: "НСБУ", bp_plan: "БП", bp: "БП" } as Record<string, string>)[s || ""] || "";
}
function srcTitle(s?: string | null): string {
  return ({
    // 'ytd' двух природ: связанные с БП строки — нарастающий итог (Q4 = год,
    // кварталы БП хранятся YTD); собственные KPI — Σ квартальных значений.
    ytd: "Факт по кварталам: для связанных с БП — нарастающий итог (Q4), для собственных KPI — сумма Q1..Q4 (годовой факт не закрыт)",
