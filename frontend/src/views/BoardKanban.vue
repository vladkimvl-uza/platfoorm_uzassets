...
async function openTaskEditor(task: TaskBrief, ev?: MouseEvent) {
  if (ev) ev.stopPropagation();
  try {
    editorEntity.value = await tasksApi.getOne(task.id);
    editorOpen.value = true;
  } catch (e: any) {
    error.value = e?.response?.data?.detail || e?.message || t("Не удалось открыть задачу");
  }
}
...
async function load() {
  ...
  try {
    ...
    error.value = e?.response?.status === 404
      ? t("Доска не найдена")
      : (e?.response?.data?.detail || e?.message || t("Не удалось загрузить доску"));
  } finally {
    loading.value = false;
  }
}
  ...
  <div class="p-8 max-w-[1600px] mx-auto">
    ...
    <template v-else-if="data">
      ...
      <nav class="flex items-center gap-2 text-xs text-slate-400 mb-4">
        <RouterLink to="/boards" class="hover:text-uza-purple">{{ t("Доски") }}</RouterLink>
        <span>›</span>
      ...
      <div class="mb-4 flex items-end justify-between flex-wrap gap-4">
        <div>
          ...
          <div class="text-xs text-slate-400 mt-1 tabular-nums">
            {{ t("{n} задач", { n: data.board.tasks_total }) }}
          </div>
        </div>
      </div>
      ...
      <div class="uza-card p-4 mb-4 flex items-center gap-6">
        <div class="flex-1">
          <div class="text-[10px] uppercase tracking-uza-label2 text-slate-500 mb-1">{{ t("Прогресс доски") }}</div>
          <div class="flex items-center gap-3">
            ...
            <div class="text-xs text-slate-500">
              <div class="tabular-nums">{{ t("{done} завершено из {total}", { done: boardProgress.done, total: boardProgress.total }) }}</div>
              <div v-if="boardProgress.excluded > 0" class="text-slate-400 mt-0.5">
                {{ t("{n} исключено (ежемес./постоянные)", { n: boardProgress.excluded }) }}
              </div>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <div class="space-y-2">
            <div
              ...
              :style="{ '--stripe-color': PRIO_COLOR[tk.priority] }"
            >
              <!-- Top row: num + priority pill -->
              <div class="flex items-start justify-between gap-2 mb-1.5">
                <span v-if="tk.num" class="text-[10px] uppercase tracking-uza-label2 text-slate-400 tabular-nums">
                  {{ tk.num }}
                </span>
                <span
                  class="text-[9px] uppercase tracking-uza-label2 font-medium px-1.5 py-0.5 rounded whitespace-nowrap"
                  :style="{ background: PRIO_COLOR[tk.priority] + '15', color: PRIO_COLOR[tk.priority] }"
                >{{ t(PRIO_LABEL[tk.priority]) }}</span>
              </div>

              <!-- Title -->
              <div class="text-sm text-slate-900 leading-snug mb-2">{{ tk.title }}</div>

              <!-- Badges — единый ряд с переносом -->
              <div
                v-if="tk.direction_meta || tk.linked_year || tk.is_project || ['quarterly','monthly','ongoing'].includes(tk.status)"
                class="flex flex-wrap items-center gap-[5px] mb-2"
              >
                <DirectionBadge v-if="tk.direction_meta" :direction="tk.direction_meta" variant="bar" size="sm" />

                <span v-if="tk.linked_year"
                      ...
                      :title="t('Перенесена из прошлого года')">
                ...
                <span v-if="tk.is_project"
                      class="inline-flex items-center px-2 py-0.5 rounded-md text-[9px] uppercase tracking-uza-label2 font-medium"
                      style="background:rgba(127,119,221,.10);color:#534AB7">{{ t("Проект") }}</span>

                <span v-if="tk.status === 'quarterly'"
                      class="inline-block px-2 py-0.5 text-[10px] rounded tabular-nums"
                      :style="isQuarterlyAllDone(tk)
                              ? { background: '#DCFCE7', color: '#0E7A58' }
                              : { background: 'rgba(168,85,247,.13)', color: '#7E22CE' }">
                  <template v-if="isQuarterlyAllDone(tk)">✓ {{ t("Все кварталы") }}</template>
                  <template v-else>{{ t("Кв") }} · {{ quarterlyDoneCount(tk) }}/4</template>
                </span>

                <span v-else-if="tk.status === 'monthly' || tk.status === 'ongoing'"
                      class="inline-block px-2 py-0.5 text-[10px] rounded text-slate-500" style="background:#F1F5F9">
                  {{ tk.status === 'monthly' ? t('Ежемесячно') : t('Постоянная') }} · {{ t('вне %') }}
                </span>
              </div>

              <!-- Входящие задачи проекта -->
              <div v-if="tk.is_project && subtasksCache[tk.id] && subtasksCache[tk.id].length"
                   class="mb-2 rounded-md px-2 py-1.5" style="background:rgba(127,119,221,.05)">
                <div class="flex items-center justify-between mb-1">
                  <span class="text-[9px] uppercase tracking-uza-label2 text-slate-400 font-medium">{{ t("Задачи") }}</span>
                  <span class="text-[9px] text-slate-400 tabular-nums">
                    {{ t("{done} из {total}", { done: subDone(tk.id), total: subtasksCache[tk.id].length }) }} · {{ subPct(tk.id) }}%
                  </span>
                </div>
                <div class="space-y-0.5">
                  <div v-for="s in subtasksCache[tk.id].slice(0, 5)" :key="s.id" class="flex items-center gap-1.5">
                  ...
                  <div v-if="subtasksCache[tk.id].length > 5" class="text-[9px] text-slate-400" style="padding-left:13px">
                    {{ t("ещё {n}", { n: subtasksCache[tk.id].length - 5 }) }}
                  </div>
                </div>
              </div>
              ...
              <div class="flex items-center justify-between gap-2 text-[10px]">
                <span v-if="tk.assignee_name || tk.assignee_email"
                      class="flex items-center gap-1.5 min-w-0"
                      :title="tk.assignee_email || ''">
                  <span class="inline-flex items-center justify-center flex-shrink-0 text-white font-medium"
                    ...
                    {{ initials(tk.assignee_name || tk.assignee_email) }}
                  </span>
                  <span class="truncate text-slate-600">{{ tk.assignee_name || tk.assignee_email }}</span>
                </span>
                <span v-else class="text-slate-300">{{ t("не назначена") }}</span>

                <span
                  v-if="tk.due_date"
                  class="flex items-center gap-1 flex-shrink-0 tabular-nums"
                  :class="isOverdue(tk) ? 'text-uza-red font-medium' : 'text-slate-500'"
                >
              ...
              <!-- Progress bar -->
              <div v-if="tk.progress_percent > 0" class="mt-2 h-1 bg-slate-100 rounded-uza-pill overflow-hidden">
                <div
                  class="h-full rounded-uza-pill"
                  :style="{ width: tk.progress_percent + '%', background: col.color }"
                ></div>
              </div>
            </div>
