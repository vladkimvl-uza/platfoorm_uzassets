...
async def global_search(
    ...
    for r in rows:
        results.append({
            ...
            "subtitle": r[3] or "Компания", "link": f"/library/companies/{r[0]}",
        })
    ...
    for etype, tbl in (("project", "projects"), ("task", "tasks")):
        ...
        for r in rows:
            sub = (f"№{r[2]} · " if r[2] else "") + (r[3] or ("Проект" if etype == "project" else "Задача"))
            results.append({
    ...
    for r in rows:
        results.append({
            ...
            "subtitle": "Консультант", "link": "/consultants",
        })
    ...
    for r in rows:
        results.append({
            ...
            "subtitle": r[2] or "Пользователь", "link": "/admin/rbac-v3",
        })
    ...
    for r in rows:
        results.append({
            "type": "note", "id": r[0], "title": r[1] or "Заметка",
            "subtitle": "Заметка", "link": "/calendar",
        })
