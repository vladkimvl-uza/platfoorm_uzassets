...
/**
 * ESGCompanyDetailPanel — единое ESG-представление одной компании: 4 балльные
 * карточки (E/S/G + общий), разбивка метрик по столпам с инлайн-правкой,
 * существенные вопросы. Используется И в модалке /esg (ESGCompanyDetailModal),
 * И во вкладке воркспейса (tab=esg) — один источник → гарантированный синк.
 *
 * variant="modal"    — с eyebrow, годовым селектором, кнопкой закрытия (в модалке).
 * variant="embedded" — без них (год задаёт воркспейс через initialYear).
 */
...
async function onEditorSaved() { await load(); emit("changed"); }

// ─── Инлайн-правка значения метрики (клик по значению) ───
...
async function load() {
  ...
  error.value = null;
  try {
    let d = await esgApi.getCompanyDetail(props.companyId, currentYear.value ?? undefined);
    // Year-fallback: за выбранный FY метрик нет → последний доступный год.
    if (d && _mc(d) === 0 && Array.isArray(d.available_years) && d.available_years.length) {
      const ys = [...d.available_years].sort((a: number, b: number) => b - a);
      const target = ys.find((y: number) => y <= (currentYear.value ?? ys[0])) ?? ys[0];
      if (target && target !== d.year) {
        const alt = await esgApi.getCompanyDetail(props.companyId, target).catch(() => null);
        if (alt && _mc(alt) > 0) d = alt;
      }
    }
    detail.value = d;
...
watch(() => props.initialYear, (y) => {
  if (props.variant === "embedded" && y != null && y !== currentYear.value) {
    currentYear.value = y;
    load();
  }
});
...
function onYearChange(e: Event) {
  ...
  if (!isNaN(v)) { currentYear.value = v; load(); }
}
...
const isEmpty = computed(() => detail.value != null && _mc(detail.value) === 0 && (detail.value.issues?.length || 0) === 0);
</script>
    ...
    <template v-else-if="detail">
      ...
      <div class="ec-header">
        ...
        <div class="ec-header-r">
          <select
            v-if="variant === 'modal' && detail.available_years.length > 1"
            :value="String(detail.year)" @change="onYearChange" class="ec-year-sel"
          >
...
<style scoped>
.ec-panel { display: flex; flex-direction: column; }
.ec-panel--embedded { gap: 0; }
...
.ec-panel--embedded .ec-header { padding: 2px 2px 14px; border-bottom: none; }
...
.ec-year-sel { font: inherit; font-size: 11px; padding: 4px 9px; border: 1px solid rgba(15, 23, 60, .12); border-radius: 5px; background: var(--bg1, #fff); font-feature-settings: 'tnum'; outline: none; font-family: inherit; }
...
.ec-edit-btn {
  display: inline-flex; align-items: center; padding: 6px 13px; border: none; border-radius: 9px;
...
.ec-pillars {
  ...
  padding: 14px 22px; border-bottom: 1px solid rgba(15, 23, 60, .06);
}
...
.ec-pillar-h { display: flex; align-items: center; gap: 6px; margin-bottom: 4px; }
.ec-pillar-letter { width: 18px; height: 18px; border-radius: 4px; color: #fff; font-size: 11px; font-weight: 700; display: flex; align-items: center; justify-content: center; }
.ec-pillar-l { font-size: 9.5px; font-weight: 600; letter-spacing: .04em; text-transform: uppercase; color: rgba(15, 23, 60, .65); }
.ec-pillar-score { font-size: 22px; font-weight: 400; letter-spacing: -.025em; font-feature-settings: 'tnum'; }
.ec-pillar-of { font-size: 11px; opacity: .55; font-weight: 500; margin-left: 1px; }
.ec-pillar-count { font-size: 10px; color: rgba(15, 23, 60, .55); }

.ec-body { flex: 1; overflow-y: auto; padding: 16px 22px 24px; display: flex; flex-direction: column; gap: 18px; }
.ec-panel--embedded .ec-body { padding: 16px 0 0; overflow: visible; }
.ec-sec-h { font-size: 10px; font-weight: 600; letter-spacing: .07em; text-transform: uppercase; color: rgba(15, 23, 60, .55); margin-bottom: 8px; }
.ec-sec-h-small { font-size: 10px; font-weight: 500; text-transform: none; letter-spacing: 0; color: rgba(15, 23, 60, .65); margin-left: 4px; }

.ec-metrics-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; }
@media (max-width: 900px) { .ec-metrics-grid { grid-template-columns: 1fr; } }
.ec-pillar-col { background: var(--bg2, #FAFAFD); border-radius: 9px; padding: 12px; }
.ec-col-h { display: flex; align-items: center; gap: 6px; font-size: 10px; font-weight: 600; letter-spacing: .04em; text-transform: uppercase; margin-bottom: 8px; }
.ec-col-dot { width: 8px; height: 8px; border-radius: 50%; }
.ec-metric-list { display: flex; flex-direction: column; gap: 8px; }
.ec-metric { background: var(--bg1, #fff); border-radius: 6px; padding: 8px 10px; border: 1px solid rgba(15, 23, 60, .04); }
...
.ec-metric-code { font-family: 'SF Mono', 'Menlo', monospace; font-size: 9px; color: rgba(15, 23, 60, .45); }
...
.ec-metric-input { width: 92px; box-sizing: border-box; border: 1.5px solid var(--p, #7C6FF7); border-radius: 7px; background: var(--bg2, #F8FAFC); padding: 3px 7px; font-size: 13px; font-family: inherit; color: var(--t1, #1E2A4A); outline: none; }
.ec-metric-input:focus { box-shadow: 0 0 0 3px rgba(124, 111, 247, .14); }
.ec-metric-iok, .ec-metric-ix { width: 22px; height: 22px; flex-shrink: 0; display: inline-flex; align-items: center; justify-content: center; border: none; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: 700; font-family: inherit; }
...
.ec-metric-att { font-size: 12px; font-weight: 600; font-feature-settings: 'tnum'; }
.ec-metric-bar { margin-top: 6px; height: 4px; background: rgba(15, 23, 60, .06); border-radius: 2px; overflow: hidden; }
.ec-metric-bar-fill { height: 100%; border-radius: 2px; background-image: linear-gradient(180deg, rgba(255, 255, 255, 0.30) 0%, rgba(255, 255, 255, 0) 55%); transition: width .8s var(--ease-standard); }

.ec-issue-list { display: flex; flex-direction: column; gap: 8px; }
.ec-issue { background: var(--bg2, #FAFAFD); border-radius: 6px; padding: 9px 12px 9px 18px; position: relative; overflow: hidden; }
.ec-issue::before { content: ""; position: absolute; left: 6px; top: 6px; bottom: 6px; width: 4px; border-radius: 4px; background: var(--stripe-color, #94A3B8); pointer-events: none; }
.ec-issue-h { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; font-size: 11px; }
.ec-issue-pillar { width: 18px; height: 18px; border-radius: 4px; font-size: 10px; font-weight: 700; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.ec-issue-title { font-weight: 500; color: var(--t1, #1e2a4a); flex: 1; min-width: 0; }
.ec-issue-sev { font-size: 10px; font-weight: 600; }
.ec-issue-status { font-size: 9.5px; font-weight: 600; padding: 1px 6px; border-radius: 3px; letter-spacing: .04em; text-transform: uppercase; }
.ec-issue-d { font-size: 11px; color: rgba(15, 23, 60, .65); margin-top: 4px; line-height: 1.45; }
</style>
