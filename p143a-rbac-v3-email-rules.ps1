# =====================================================================
# p143a-rbac-v3-email-rules.ps1   (RBAC v3 session 4 - part 1)
# =====================================================================
# Implements EmailRulesPage with full CRUD via existing endpoints:
#   GET    /rbac/role-by-email        -> RoleByEmailRule[]
#   POST   /rbac/role-by-email        -> RoleByEmailRule
#   DELETE /rbac/role-by-email/{id}   -> 204
#
# These rules pre-assign roles to email addresses. When a new user signs
# up with a matching email, they auto-get those roles + scoping.
# Useful for: domain-wide policies (e.g. *@uz-assets.uz -> readonly),
# named-employee pre-provisioning, contractor onboarding.
# =====================================================================
...
function Write-NewFile($path, $content, $label, [switch]$Overwrite) {
    ...
    if (Test-Path -LiteralPath $path) {
        ...
        Copy-Item -LiteralPath $path -Destination "$path.bakP143a.$stamp" -Force
    }
...
# ───────────────────────────────────────────────────────────────────────
# [1/2] api/rbacV3.ts — add emailRulesApi
# ───────────────────────────────────────────────────────────────────────
$apiAddon = @'
...
export const emailRulesApi = {
  async list(): Promise<RbacV3EmailRule[]> {
    const { data } = await api.get<RbacV3EmailRule[]>('/rbac/role-by-email');
    return data;
  },
  async create(payload: RbacV3EmailRuleCreate): Promise<RbacV3EmailRule> {
    const { data } = await api.post<RbacV3EmailRule>('/rbac/role-by-email', payload);
    return data;
  },
  async remove(id: string): Promise<void> {
    await api.delete(`/rbac/role-by-email/${id}`);
  },
};
'@
...
if ($apiSrc.Contains('export const emailRulesApi')) {
    Write-Host "[*] [1/2] api: emailRulesApi already added" -ForegroundColor DarkGray
} else {
    Copy-Item -LiteralPath $apiPath -Destination "$apiPath.bakP143a.$stamp" -Force
    Write-Host "[*] [1/2] api/rbacV3.ts: adding emailRulesApi" -ForegroundColor Yellow
    Write-File $apiPath ($apiSrc + $apiAddon)
    Write-Host "    OK" -ForegroundColor Green
}

# ───────────────────────────────────────────────────────────────────────
# [2/2] views/rbac-v3/EmailRulesPage.vue
# ───────────────────────────────────────────────────────────────────────
$page = @'
...
'@
Write-NewFile (Join-Path $root "$fe\views\rbac-v3\EmailRulesPage.vue") $page "[2/2] views/rbac-v3/EmailRulesPage.vue" -Overwrite
...
if (-not $fec) {
    ...
    try {
        ...
        # CSS warnings are not failures
        Write-Host "    (warnings ignored)" -ForegroundColor DarkGray
    }
...
Write-Host " p143a COMPLETE - Email rules page live" -ForegroundColor Green
Write-Host "===================================================================" -ForegroundColor Green
Write-Host ""
Write-Host "Test (Ctrl+Shift+R):" -ForegroundColor Cyan
Write-Host "  1. /admin/rbac-v3 -> tab Email-правила" -ForegroundColor White
Write-Host "  2. If no rules yet: empty-state card with usage hint" -ForegroundColor White
Write-Host "  3. Green '+ Новое правило' button top-right opens modal" -ForegroundColor White
Write-Host "  4. Create '*@uz-assets.uz' -> readonly role -> appears in list" -ForegroundColor White
Write-Host "  5. PATTERN badge highlights wildcard rules" -ForegroundColor White
Write-Host "  6. Trash icon on each row deletes with confirm prompt" -ForegroundColor White
